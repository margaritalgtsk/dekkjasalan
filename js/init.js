...
jQuery(document).ready(function(){
	...
	jQuery('.off-canvas-menu-container').after('<div id="search-3" class="widget widget_search"><h4>Leita að vörum</h4><form role="search" method="get" class="search-form" action="/"><input type="text" class="search-field" placeholder="Leita á vef" value="" name="s" title="Leita á vef"><input type="submit" class="search-submit" value="Leita"></form></div>');
	
	jQuery('#post-area article .content-inner>a').append('<div class="magnifer"></div>');
	
	jQuery('[name="skilabod"]').val('Spurning um '+jQuery('.blog-title .entry-title').text());
	
	
	var initSmallThumbnail = function(){
		
		var htmlThumbnails = '<ul class="easy-gallery-thumbnail">'
		var i=0
		jQuery('.image-gallery img').each(function(){
			
			var thumbnailGallery = 	jQuery(this).attr('src');
			htmlThumbnails+='<li><a href="" data-thumb="'+i+'"><img style="width:80px" src="'+thumbnailGallery+'" /></a></li>';
		...
		jQuery('.image-gallery').after(htmlThumbnails);
		
		jQuery('.easy-gallery-thumbnail li').eq(0).addClass('active');
		
		jQuery('.easy-gallery-thumbnail a').click(function(e){

			e.preventDefault();
			var thumbNumber = jQuery(this).data('thumb');
			
			jQuery('.image-gallery li').hide();
			jQuery('.easy-gallery-thumbnail  li').removeClass('active');
			jQuery('.image-gallery li').eq(thumbNumber).show();
	...
	initSmallThumbnail();
	jQuery(".posts-container article a").on('click',function(){
...
/************ #custom Disable goto link from gallery *************/

    var galleryItem = $('.gallery .gallery-item a');
	if (galleryItem[0] != undefined) {
		console.log('Gallery item exist');
		setTimeout(function() {
